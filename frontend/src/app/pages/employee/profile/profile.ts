        ...
        <div class="left-column">
          <div class="profile-card">
            <div class="avatar-section">
              ...
              <p class="role"><i class="pi pi-briefcase"></i> {{ currentUser?.roles?.[0] || 'Employee' }}</p>
            </div>
          </div>
          ...
          <div class="info-card">
            ...
            <div class="info-grid">
              ...
              <div class="info-item">
                ...
                <span>{{ employeeId || 'EMP' + currentUser?.id }}</span>
              </div>
          ...
          <div class="activity-card">
            ...
            <div class="activity-stats">
              ...
              <div class="stat-item">
                ...
                <div class="stat-label"><i class="pi pi-clock"></i> Upcoming</div>
              </div>
              <div class="stat-item">
                <div class="stat-number">{{ thisMonthMeetings }}</div>
                <div class="stat-label"><i class="pi pi-calendar-plus"></i> This Month</div>
              </div>
            </div>
          </div>
  ...
  loadUserStats(): void {
    if (this.currentUser) {
      this.bookingService.getBookingsByUser(this.currentUser.id).subscribe({
        next: (bookings) => {
          this.totalMeetings = bookings.length;
          const now = new Date();
          this.upcomingMeetings = bookings.filter(b => new Date(b.startTime) > now).length;
          const thisMonth = new Date().getMonth();
          this.thisMonthMeetings = bookings.filter(b => new Date(b.startTime).getMonth() === thisMonth).length;
        },
        error: () => {
          this.totalMeetings = 0;
          this.upcomingMeetings = 0;
          this.thisMonthMeetings = 0;
        }
      });
    }
  }
